...
public class ToPicReferenceUtil4 implements Runnable {
	...
	public ToPicReferenceUtil4(String fileRealPath, String upload, String isflag,String savepath, String resType, HashMap paramMap, String dsname) {
		super();
	...
	public void run() {
		...
		try {
			...
			paramMap.put(roomType+"RelLinksState", "处理完成");
			
		} catch (Exception e) {
			// TODO Auto-generated catch block
			paramMap.put(roomType+"RelLinksState", "处理失败");
		} 
		try {
			SqlSession sqlsession=SessionFactory.getInstance().getSQLSession(dsname);
			
			String relLinksState = "";
			
			if(isflag.equals("isrellinks")){
				relLinksState = resType+".update_relLinksState";
			}
			
			int r=sqlsession.update(relLinksState, paramMap) ;
