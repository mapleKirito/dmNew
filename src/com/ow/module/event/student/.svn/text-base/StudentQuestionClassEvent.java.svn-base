...
public class StudentQuestionClassEvent implements EventListener {
	static Logger log = Logger.getLogger(Constant.FRAMEWORK);
	private ResultData resultData;
	public StudentQuestionClassEvent() {
	...
	public ResultData event(EventExecutor eventExecutor) throws Exception {
		try {
			...
			String cpage = paramMap.get("questionCurrentClassPage") == null ? "1" : paramMap.get("questionCurrentClassPage").toString();
			...
			//当前页
			int questionCurrentClassPage = 1;
			String ucpage = eventExecutor.getBaseAction().getCurrentPage();
			try{
				questionCurrentClassPage = Integer.parseInt(ucpage);
				
				if(!change_type.equals("4") ){
					questionCurrentClassPage = 1;
			...
			paginatedData.setCurrentPage(questionCurrentClassPage);
			paginatedData.setPageCount(pageCount);
			paginatedData.setPageSize(pageSize);
			paginatedData.setSelfPageName("paramMap.questionCurrentClassPage");
			paginatedData.setContextName("questionClassPaginated");
		...
		catch (Exception e)
		{
			...
			String error = DetailException.expDetail(e, StudentQuestionClassEvent.class);
			log.error(error);
		}
		return resultData;
	}
