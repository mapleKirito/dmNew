...
public class TeamDelEvent implements EventListener {
	...
	public ResultData event(EventExecutor eventExecutor) throws Exception {
		try
		{
			...
			String delT = eventExecutor.getSqlidByIndex(0);
			String delM = eventExecutor.getSqlidByIndex(1);
			int ans=eventExecutor.getSqlSession().delete(delT, paramMap);
			json.put("flag", ans);
			if(ans>0){
				ans=eventExecutor.getSqlSession().delete(delM, paramMap);
		...
		catch (Exception e)
		{
			...
			String error = DetailException.expDetail(e, TeamDelEvent.class);
			log.error(error);
		}
		return resultData;
	}
