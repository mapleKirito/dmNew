...
$(document).ready(function(){
	...
	$(".rc-qrcode").click(function(){
		...
		var st=localhostPaht+"/dm/page/scanner/"+"${initMap.picDetail.prNo}"+"/scanner.html";
		art.dialog.data('str', st);
		art.dialog.data('code', "${initMap.picDetail.prNo}");
		art.dialog.data('contextPath', '<%=request.getContextPath()%>');
	...
	//$("#revL").load("decorateForwardAction.action?code=-600604&paramMap.ptype=observation&paramMap.pid="+$("#reviewID").val()+"&paramMap.resURL="+$(".addFavorite").attr("id")+"&paramMap.userID=${paramMap.s_userID}&paramMap.userType=${paramMap.s_userType}");
       $("#revR").click(function(){
		...
		art.dialog.data('ptype', "pbservation");
  ...
  //音频控制
    ...
    if(ua.indexOf("msie")>0){
	   $("#bgm").html('<embed src="${initMap.picDetail.prAudio}" id="mp" style="height:0px;width:0px;" autostart="false" loop="true">');
	}else{
		$("#bgm").html('<audio src="${initMap.picDetail.prAudio}" id="mp"></audio>');
	}	

   
    $.ajax({
		url:		'ajaxAction.action?suiji='+new Date().getTime()+'&code=${code}&paramMap.roomtype=projection&paramMap.prID=${initMap.picDetail.prID}',
		type:	    'post',
		timeout:	'60000',
		data:	Array(),
		dataType:	'json',
...
function fieldControl(){
	...
	if(isFieldNull('${initMap.picDetail.prCode}')) $(".erCode").parent().remove();
    
    if(isFieldNull('${initMap.picDetail.prCategoryName}')) $(".prCategoryName").parent().remove();
    
    if(isFieldNull('${initMap.picDetail.prCateTwLevelName}')) $(".prCateTwLevelName").parent().remove();
    
    if(isFieldNull('${initMap.picDetail.prCateTHLevelName}')) $(".prCateTHLevelName").parent().remove();
    
    if(isFieldNull('${initMap.picDetail.prCateFoLevelName}')) $(".prCateFoLevelName").parent().remove();
    
    
    //初始化 国家 tr
    if(isFieldNull('${initMap.picDetail.prCountryName}')) $(".prCountryName").parent().remove();
    
    //初始化 省份 tr
    if(isFieldNull('${initMap.picDetail.prProvinceName}')) $(".prProvinceName").parent().remove();
    
    //初始化 市县区 tr
    if(isFieldNull('${initMap.picDetail.prCity}') || isFieldNull('${initMap.picDetail.prCity}')==isFieldNull('${initMap.picDetail.prProvinceName}')) $(".prCity").parent().remove();
    
    //初始化 出土地 tr
    if(isFieldNull('${initMap.picDetail.prPlaceName}')) $(".prPlaceName").parent().remove();
    
    
    //初始化 备注 tr
    if(isFieldNull('${initMap.picDetail.prRemarks}')) $(".prRemarks").parent().remove();
...
<input type="hidden" class="roomname" id="roomname" value="projection">
<input type="hidden" id="prName" value="${initMap.picDetail.prName}">
<%-- <input type="hidden" id="prTotal" value="${initMap.picDetail.prTotal}"> --%>
...
<input type="hidden" id="userType" value="${paramMap.s_userType}">
 ...
 <div id="layout_mainbody"> 
 	 <div class="res-detail-info">
                   <h4 class="res_name" style="background:url(<%=request.getContextPath()%>/images-ce/detaltop.png) center bottom no-repeat; background-size:100% 100%;background-image: url('<%=request.getContextPath()%>/images-ce/detaltop.png');background-size: cover;"><b>资源名称：</b><label>${initMap.picDetail.prName}</label></h4>
				                       ...
				                       <li title="下载">
						                  	...
						                  	<div class="hide-popup popup-rc-cloud-down">
						                          ...
						                          <div class="hide-popup-content">
						                              <table><tr>
						                              		<s:if test="initMap.picDetail.prAudio != ''&&initMap.picDetail.prType!='1073' "> 
						                                      <td class="key keya"><a href="downloadAction.action?code=-706&paramMap.resDownRoom=observation&paramMap.resDownType=1073&paramMap.resPath=${initMap.picDetail.prAudio}">音频下载</a></td>
						                                	
						                                 	 </s:if>
						                                  
						                                      <td class="key keya"><a  id="download" href="downloadAction.action?code=-706&paramMap.resDownRoom=observation&paramMap.resDownType=${initMap.picDetail.prType}&paramMap.resPath=${initMap.picDetail.prUpload}">资源下载</a></td>
						                                  </tr>
						                              </table>
						                          </div>
				                         ...
				                         <s:if test="session.s_userType != 3 and paramMap.st == 1 and session.s_userType != 5">
				                  			 <li>
				                  			 	<a  data-type="rc-query" title="加入收藏" onclick="window.parent.insert_fav(this)" class="addFavorite rc rc-query"  href="javascript:void(0);"   id="${code}|${paramMap.path}|observation|${paramMap.st}|${initMap.picDetail.prName}|${initMap.picDetail.prType}|${initMap.picDetail.prID}" >
				                  				</a>  
				                  			</li>
				                  		</s:if>
						                <!-- 二维码 -->
				                        ...
				                        <li>
				                        	<a href="javascript:void(0);" title="评论" data-type="rc-comment" class="rb rc-comment" id="revR" ></a>
												...
												<input type="hidden" id="reviewID" value="${initMap.picDetail.prID}">
											</div>
				                            ...
				                            <div class="hide-popup popup-rc-info">
				                                ...
				                                <div class="hide-popup-content">
				                                 ...
				                                 <tr>
														...
														<td width="75%" align="center" class="orCode">${initMap.picDetail.prNO}</td>
													</tr>
													<tr>
														<td width="25%" align="center">资源名称</td>
														<td width="75%" align="center" class="orName">${initMap.picDetail.prName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">资源类型</td>
														<td width="75%" align="center" class="orType">${initMap.picDetail.prType eq "1011" ? '高清图片':(initMap.picDetail.prType eq "1073" ? '音频':(initMap.picDetail.prType eq "4033" ? '压缩包':'PDF')) }</td>
													</tr>
													<tr>
														<td width="25%" align="center">一级分类</td>
														<td width="75%" align="center" class="orCategoryName">${initMap.picDetail.prCategoryName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">二级分类</td>
														<td width="75%" align="center" class="orCateTwLevelName">${initMap.picDetail.prCateTwLevelName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">三级分类</td>
														<td width="75%" align="center" class="orCateTHLevelName">${initMap.picDetail.prCateTHLevelName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">四级分类</td>
														<td width="75%" align="center" class="orCateFoLevelName">${initMap.picDetail.prCateFoLevelName}</td>
													</tr>
													 <tr>
														<td width="25%" align="center">国家</td>
														<td width="75%" align="center" class="orCountryName">${initMap.picDetail.prCountryName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">省份</td>
														<td width="75%" align="center" class="orProvinceName">${initMap.picDetail.prProvinceName}</td>
													</tr> 
													<%--  <tr>
														<td width="25%" align="center">市区县</td>
														<td width="75%" align="center" class="orCity">${initMap.picDetail.prCityName}</td>
													</tr>  --%>
													<tr>
														<td width="25%" align="center">出土地</td>
														<td width="75%" align="center" class="orPlaceName">${initMap.picDetail.prPlaceName}</td>
													</tr>
													<tr>
														<td width="25%" align="center">备注</td>
														<td width="75%" align="center" class="orRemarks">${initMap.picDetail.prRemarks}</td>
													</tr>
                        ...
                        <div class="relative-res-body">
                            ...
                            <div class="relative-res-container">
                                ...
                                <ul class="res_t" id="relative-res-body_res_t">
                                  ...
                                  <li idInfo='${initMap.picDetail.prID}'>
					     				<a href="javascript:void(0);" audio="${initMap.picDetail.prAudio}" position="${initMap.picDetail.prCityName}"  name="${initMap.picDetail.prName}"  reviewID="${initMap.picDetail.prID}" code="${initMap.picDetail.prCode}" upload="${initMap.picDetail.prUpload}"  inThum="${initMap.picDetail.prInThum}"  params="paramMap.path=${initMap.picDetail.prPath}&paramMap.erType=${initMap.picDetail.prFileType}&paramMap.fileType=${initMap.picDetail.prFileType}&paramMap.erID=${initMap.picDetail.prID}&paramMap.st=1&paramMap.temppname=erID&paramMap.temppid=${initMap.picDetail.prID}&paramMap.name=${initMap.picDetail.prName}&paramMap.roomtype=exhibition" >
					     					<img src="${initMap.picDetail.prThumbnail}" onclick="preImg(2,this)"  _index="0"  alt="${initMap.picDetail.prName}"  width="77" height="77"/> 
					     				</a>
					     				<span>
						     				<s:if test="#initMap.picDetail.prName.length() > 4"><s:property value="#initMap.picDetail.prName.substring(0,4)" />...</s:if>
						     				<s:else><s:property value="#initMap.picDetail.prName" /></s:else>
						     			</span>
				     				</li>	
                                   <s:iterator value="initMap.resView" id="fv" status="fvindx">
										<s:if test="#fv.thumbnail!=null">										
						     				<li idInfo='${fv.id}'>
						     				<a href="javascript:void(0);" audio="${fv.audio}"   name="${fv.name}"  reviewID="${fv.id}" code="${fv.code}" upload="${fv.upload}"  inThum="${fv.inThum}"  params="paramMap.path=${fv.path}&paramMap.prType=${fv.fileType}&paramMap.fileType=${fv.fileType}&paramMap.orID=${fv.id}&paramMap.st=1&paramMap.temppname=orID&paramMap.temppid=${fv.id }&paramMap.name=${fv.name}&paramMap.roomtype=observation" >
