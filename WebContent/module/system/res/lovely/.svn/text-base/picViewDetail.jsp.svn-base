...
$(document).ready(function(){	
	...
	$(".rc-qrcode").click(function(){
		...
		var st=localhostPaht+"/dm/page/scanner/"+"${initMap.detail.crNo}"+"/scanner.html";
...
<input type="hidden" id="crTotal" value="${initMap.detail.crTotal}">
...
<input type="hidden" id="userType" value="${paramMap.s_userType}">
 <div id="layout_mainbody">
 	 <div class="res-detail-info">
                     <h4 class="res_name" style="background:url(<%=request.getContextPath()%>/images-ce/detaltop.png) center bottom no-repeat; background-size:100% 100%;background-image: url('<%=request.getContextPath()%>/images-ce/detaltop.png');background-size: cover; "><b>资源名称：</b><label>${initMap.detail.crName}</label></h4>
                                ...
                                <ul class="res_t" id="relative-res-body_res_t">
                                   <s:iterator value="initMap.resView" id="fv" status="fvindx">
										<s:if test="#fv.thumbnail!=null">										
						     				<li idInfo='${fv.id}'>
						     				<a href="javascript:void(0);" name="${fv.name}"  reviewID="${fv.id}" code="${fv.code}" upload="${fv.upload}"  inThum="${fv.thumbnail}"  params="paramMap.path=${fv.path}&paramMap.crType=${fv.fileType}&paramMap.fileType=${fv.fileType}&paramMap.crID=${fv.id}&paramMap.st=1&paramMap.temppname=crID&paramMap.temppid=${fv.id }&paramMap.name=${fv.name}&paramMap.roomtype=lovely" >
						     					<img src="${fv.thumbnail}"  onclick="preImg(2,this)"  _index="${fvindx.index }"  alt="${fv.name}"   title="${fv.name}" width="77" height="77"/> 
						     				</a>
						     				<span>
