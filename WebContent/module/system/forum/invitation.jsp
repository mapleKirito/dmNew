...
$(document).ready(function(){	
	$('.reply').screenTB({
		...
		buttons:'no',
		dialogType:'iframe',
		iframeSrc:'forwardAction.action?code=002001004001'
		},{
			...
			if (data.returnValue == 1){
				$.extend(data,{returnValue:'true'});
   				...
   				$.ajax({
   					...
   					url:		'ajaxAction.action?code=3002001004005&paramMap.isRecommend=${session.s_userID}&paramMap.taskId=${initMap.invitation.taskId}',
			...
			if (data.returnValue == 1){
				$.extend(data,{returnValue:'true'});
   				...
   				$.ajax({
   					...
   					url:		'ajaxAction.action?code=3002001004005&paramMap.isRecommend=0&paramMap.taskId=${initMap.invitation.taskId}',
			...
			if (data.returnValue == 1){
				$.extend(data,{returnValue:'true'});
   				$.ajax({
   					...
   					url:		'ajaxAction.action?code=3002001004007&paramMap.taskId=' +tId+ '&paramMap.erID=' + resultId,
   					...
   					success:function(jsonData,textStatus){
   						...
   						if (textStatus == "success"){
   							var locaflag = '${paramMap.locaflag}'
   							if(locaflag=='my_info_mgr')
   								locaflag = '002001';
   							else if(locaflag=='result_mgr')
   								locaflag = '002011';
   							location.href = "decorateForwardAction.action?code="+locaflag+"&"+ new Date();
   						}
   					},
			...
			if (data.returnValue == 1){
				$.extend(data,{returnValue:'true'});
   				...
   				$.ajax({
   					...
   					url:		'ajaxAction.action?code=3002001004006&paramMap.isLock=${session.s_userID}&paramMap.taskId=${initMap.invitation.taskId}',
			...
			if (data.returnValue == 1){
				$.extend(data,{returnValue:'true'});
   				...
   				$.ajax({
   					...
   					url:		'ajaxAction.action?code=3002001004006&paramMap.isLock=0&paramMap.taskId=${initMap.invitation.taskId}',
											...
											<td valign="top" bgcolor="#f5f9fc">
												<table width="100%" border="0" cellspacing="0" cellpadding="0" style="border-bottom:1px solid #cdcdcd;">
												  <tr align="center" height="30">
											        ...
											        <td width="25%">未完成作业人数：<span class="font_3"><b>${initMap.invitation.taskUnfinishedCount}</b></font></span></td>
												  </tr>
												</table>
											</td>
										</tr>
											...
											<td bgcolor="#f5f9fc">
											        ...
											        <span class="span2 font_2">
												        ...
												        <span class="span2 font_2">
												        	...
												        	<s:if test="(initMap.invitation.isLock != #session.s_userID)&&(initMap.invitation.isLock != 0) && (#session.s_userType!=3)">已锁定</s:if>
												        	<s:if test="initMap.invitation.isLock != 0 &&((initMap.invitation.isLock == #session.s_userID) ||(#session.s_userType==3))"><a href="javascript:void(0)" class="invUnlock" id="${initMap.invitation.taskId}" name="${initMap.invitation.isLock}">解锁</a></s:if>
												        </span>
											        </td>
										...
										<td >
						                    ...
						                    <table border="0" cellspacing="0" cellpadding="0" class="table_611_1 margin_top1"  width="100%">
											  ...
											  <tr>
												...
												<td width="100%" valign="top" bgcolor="#fdfeff" style="padding-top:10px;text-indent:2em ">
						                        <p class="lineheight_1" >
