function flow(mh, mv) {//参数mh和mv是定义数据块之间的间距，mh是水平距离，mv是垂直距离 
	...
	document.getElementById("loadimg").style.top = _getMaxValue(oArr) + 250 + "px";//将loading移到下面
		...
		if (scrollTop >= st - document.documentElement.clientHeight) { 
			...
			_request(null, "Pic_init", function(data) {//当滚动到达最短的一列的距离时便发送ajax请求新的数据，然后执行回调函数 
				_addItem(data.d, function() {//追加数据 
					...
					document.getElementById("loadimg").style.top = _getMaxValue(oArr) + 250 + "px";//loading向下移位 
					liLen = liLenNew; 
	...
	(function loadimg() { 
		...
		img.onload = function() { 
			...
			for (var k in arr) { 
				var img = new Image(); 
				img.src = arr[k].img; 
				_html += '<li><img src="' + arr[k].img + '" /><a href="javascript:void(0)">' + arr[k].title + '</a></li>'; 
			} 
