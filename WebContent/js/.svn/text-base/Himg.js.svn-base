...
function img_init() {	
		$("#resView").die().live("click",function() {
			LookImgFun();
			/*img_show('', $("#resView").attr("upload"), false);
			this.blur();*/
			return false;
		});		
}
function img_show(caption, url, imageGroup,flag) {
	try {
		...
		if (iv_detectMacXFF()) {
			$("#IV_overlay").addClass("IV_overlayMacFFBGHack");
		} else {
			$("#IV_overlay").addClass("IV_overlayBG");
		}
		if (caption === null) {
			caption = "";
		}
		imgvwBox().append("<div id='IV_load'>loading..</div>");
		$('#IV_load').show();
		var baseURL;
		if (url.indexOf("?") !== -1) {
		...
		if (urlType == '.jpg' || urlType == '.jpeg' || urlType == '.png' || urlType == '.gif' || urlType == '.bmp') {
			...
			imgPreloader.onload = function() {
				...
				IV_WIDTH = x;
				IV_HEIGHT = y + 60;
				var toolbar = "<div id='IV_toolbar'><a class='IV_ico IV_zoomIn' title='缩小'></a><a style='margin-right:100px;' class='IV_ico IV_zoomOut' title='放大'></a><a class='IV_ico IV_zoomAuto' title='实际大小'></a><a class='IV_ico IV_zoomFix' title='合适大小' style='margin-right:100px;'></a></div>";
				$("#IV_window").html("");
				$("#IV_window").empty();
				$("#IV_window").append("<div id='IV_header'>"+toolbar +"<div id='IV_caption'><b>" + caption + "</b><div id='IV_secondLine'>" + IV_imageCount + IV_PrevHTML + IV_NextHTML + "</div></div>" +  "<div id='IV_closeWindow'><a href='#' id='IV_closeWindowButton' style='height:40px;width:40px;' title='Close'></a></div></div>" + "<img id='IV_Image' src='" + url + "' width='" + imageWidth + "' height='" + imageHeight + "' alt='" + caption + "'/>");
				
				if(caption.length<7){
					$("#IV_caption b").css("margin-left","100px");
				}
				
				$("#IV_closeWindow").click(iv_remove);
				if (! (IV_PrevHTML === "")) {
					function goPrev() {
...
function zoomN(o) {
	var u=$(o).attr("src");
	var max=max_*1;
	var num=u.substring(u.lastIndexOf("/")+1, u.length-4);
	num=num*1+1;
	if(num>max){
		num=1;
	}
	var pic_url=url_;
	$(o).attr("src",pic_url+"/"+num+".jpg");
	updatePosition(o);
	return false;
}
function zoomB(o) {
...
function iv_position() {
	var deailimgwidth = document.body.clientWidth;
	$("#IV_window").css({
		marginLeft: '-' + (parseInt((IV_WIDTH / 2), 10))-8 + 'px',
		width: deailimgwidth + 'px',
		height: IV_HEIGHT + 'px'
	});
	if (! (jQuery.browser.msie && jQuery.browser.version < 7)) {
		$("#IV_window").css({
			marginTop: '-' + parseInt((IV_HEIGHT / 2), 10) + 'px'
		});
	}
}
function iv_remove() {
	$("#IV_closeWindow").unbind("click");
	$("#IV_window").fadeOut("fast",
	function() {
		$('#IV_window,#IV_overlay,#IV_HideSelect').trigger("unload").unbind().remove();
	});
	$("#IV_load").remove();
	if (typeof document.body.style.maxHeight == "undefined") {
		$("body", "html").css({
