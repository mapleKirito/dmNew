...
//opt：     相关操作对外开放配置或扩展
//初始化数据
$.fn.gridTreeTB2 = function(data,opt,initData){
				            	...
				            	if (GRIDTREE.data.treeCheckbox && GRIDTREE.data.treeCheckbox == 'true'){
									...
									if (boo){
										$input = $("<input type='checkbox' class='jq_treeCheckbox' "
				            				+ " name='paramMap.treeCheckboxVal' id='CHECKBOX_" +idxv+"' value='"
...
//						alert("opid---->")+opid;
						var data = GRIDTREE.data.ajax.data + "&paramMap.orgParentID=" + opid;
						var pjson = {
							data: data,	
							beforeSend:function(request){},
							success:function(jsonData,textStatus){
...
//				alert("orgParentID--->"+GRIDTREE.data.treeNodeStartValue);
				GRIDTREE.ajax.data = GRIDTREE.data.ajax.data + "&paramMap.orgParentID=" + GRIDTREE.data.treeNodeStartValue + "&" + GRIDTREE.data.globalParam ;				
