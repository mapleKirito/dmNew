...
(function($){
	...
	view = {
		...
		appendNodes: function(setting, level, nodes, parentNode, initFlag, openFlag) {
			...
			for (var i = 0, l = nodes.length; i < l; i++) {
				...
				if (openFlag) {
					...
					var temp=["文物博览室", "专题研读室", "教学研究室" ,"影像资料室", "自主探究室"];
					...
					if(temp.indexOf(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'))<0){
						//alert(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')+"=="+node.level);
						if("中国"==node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;') || "其它地区"==node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')){
							html.push("<a id='", node.tId, consts.id.A, "' class='area_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
						...
						if(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')=="文物博览室"){
							html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level," exhibition_bg_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
									"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
									"'");
						}
						if(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')=="专题研读室"){
							html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level," observation_bg_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
									"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
									"'");
						}
						if(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')=="教学研究室"){
							html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level," laboratory_bg_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
									"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
									"'");
						}
						if(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')=="影像资料室"){
							html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level," projection_bg_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
									"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
									"'");
						}
						if(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;')=="自主探究室"){
							html.push("<a id='", node.tId, consts.id.A, "' class='level", node.level," expansion_bg_close' treeNode", consts.id.A," onclick=\"", (node.click || ''),
									"\" ", ((url != null && url.length > 0) ? "href='" + url + "'" : ""), " target='",view.makeNodeTarget(node),"' style='", fontStyle.join(''),
									"'");
						}
					}
					...
					{
						if(node[titleKey]){
						//	if(temp.indexOf(node[titleKey].replace(/'/g,"&#39;"))<0){
								html.push("title='", node[titleKey].replace(/'/g,"&#39;"),"'");
						//	}else{
						//		alert(node[titleKey].replace(/'/g,"&#39;"));
						//	}
					...
					if(temp.indexOf(node[nameKey].replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'))<0){
						...
						html.push("<button type='button' id='", node.tId, consts.id.ICON,
								"' title='' treeNode", consts.id.ICON," class='", view.makeNodeIcoClass(setting, node), " root_close' style='", view.makeNodeIcoStyle(setting, node), "'></button><span id='", node.tId, consts.id.SPAN,
								"' style=';background: url(images/tree/",node.tId, consts.id.A,".gif) no-repeat scroll 0 0 transparent;'></span>");
					}
		...
		asyncNode: function(setting, node, isSilent, callback) {
			...
			if (tools.isArray(setting.dataParam)){
				for (var i = 0; i < setting.dataParam.length; i++){
					var dp = setting.dataParam[i];
					tmpParam += (tmpParam.length > 0 ? "&": "") + "paramMap."+dp + "=" + eval("node."+dp);
				}
			}
		...
		expandCollapseNode: function(setting, node, expandFlag, animateFlag, callback) {
			...
			if (node.isParent) {
				...
				if (node.open) {
					...
					view.replaceRootClass(node, RootObj, consts.folder.OPEN);
					...
					view.replaceRootClass(node, RootObj, consts.folder.CLOSE);
					if (animateFlag == false || setting.view.expandSpeed == "") {
						ulObj.hide();
						tools.apply(callback, []);
					} else {
		...
		replaceRootClass: function(node, obj, newName) {
			if (!obj || node.isAjaxing) return;
			var tmpName = obj.attr("class");
			if (tmpName == undefined) return;
			var tmpList = tmpName.split("_");
		...
		selectNode: function(setting, node, addFlag) {
			...
			if(temp.indexOf(node[nameKey])<0){
				$("#" + node.tId + consts.id.A).addClass(consts.node.CURSELECTED);
			}else{
				/*if(node[nameKey]=="文物博览室"){
					//
					$("#" + node.tId + consts.id.A).addClass(" exhibition_bg");
				}
				if(node[nameKey]=="专题研读室"){
					//alert(2);
					$("#" + node.tId + consts.id.A).addClass(" observation_bg");
				}
				if(node[nameKey]=="教学研究室"){
					//alert(3);
					$("#" + node.tId + consts.id.A).addClass(" laboratory_bg");
				}
				if(node[nameKey]=="影像资料室"){
					//alert(4);
					$("#" + node.tId + consts.id.A).addClass(" projection_bg");
				}
				if(node[nameKey]=="自主探究室"){
					//alert(5);
					$("#" + node.tId + consts.id.A).addClass(" expansion_bg");
				}*/
		...
		setNodeLineIcos: function(setting, node) {
			...
			if(temp.indexOf(node[nameKey])<0){
				...
				icoObj.attr("class", view.makeNodeIcoClass(setting, node)+" root_open");
			}
		},
